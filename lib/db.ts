...
const STATEMENTS = [
  ...
  `create table if not exists task_rules (
     ...
     created_by text,
     created_at timestamptz not null default now())`,
  `create index if not exists tr_idx on task_rules (active, pos nulls last, id)`,
  `create table if not exists questionnaires (
     ...
     created_at timestamptz not null default now(),
     updated_at timestamptz not null default now())`,
  `create unique index if not exists qn_case_party_idx on questionnaires (lower(case_name), lower(party))`,
  `alter table questionnaires add column if not exists share_token text`,
  `alter table questionnaires add column if not exists active boolean not null default true`,
  `create unique index if not exists qn_token_idx on questionnaires (share_token)`,
  `create index if not exists qn_case_idx on questionnaires (lower(case_name))`,
  `create table if not exists questionnaire_responses (
     id bigserial primary key,
     case_name text not null,
     party text not null,
     person_name text, email text, phone text,
     submitted_text text,
     responses text,
     payload jsonb,
