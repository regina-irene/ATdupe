...
async function upsertMany(recs: any[], byId: Map<string, string>) {
  ...
  for (const rec of recs) {
    ...
    tuples.push("(" + slots.slice(0, -1).join(",") + "," + slots[slots.length - 1] + "::jsonb,'airtable',now(),now())");
  }
  await db()(
    `insert into tasks (airtable_id, client_name, case_name, case_id, task, status, priority, who, ord, closed, due_date, link, duration, at_modified, data, source, synced_at, updated_at)
     values ${tuples.join(",")}
     on conflict (airtable_id) do update set
       ...
       duration=excluded.duration, at_modified=excluded.at_modified, data=excluded.data, synced_at=now(),
